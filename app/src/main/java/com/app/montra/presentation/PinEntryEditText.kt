package com.app.montra.presentation
...
class PinEntryEditText : AppCompatEditText {
    ...
    private fun updateColorForLines(next: Boolean) {
        if (isFocused) {
            ...
            mLinesPaint?.color = ContextCompat.getColor(context, R.color.light_20)
            if (next) {
                mLinesPaint?.color = ContextCompat.getColor(context, R.color.black)
            }
        } else {
            mLinesPaint?.strokeWidth = mLineStroke
            mLinesPaint?.color = ContextCompat.getColor(context, R.color.light_80)
        }
    }
